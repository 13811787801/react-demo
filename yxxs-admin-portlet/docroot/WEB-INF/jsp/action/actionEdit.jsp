    ...
    <script type="text/javascript">
    	var appKeys = {
    			getAppKeys:function(mode){
    				if(mode == '.dev'){
    					return [{ k: 'xcxj', v: '小初衔接课程改革项目' },{ k: 'maxinlan', v: '翼课项目' }, { k: '', v: '翻转教学'}];
    				}else{
    					return [{ k: 'xcxj', v: '小初衔接课题组' },{ k: 'maxinlan', v: '马芯兰翼课项目' }, { k: '', v: '优学向上'}];
    				}
    			}
    	};
        var fieldInfoList = [
            ...
            { name: ['url', '活动页面'],format:function(v,d){
            	if(d.url){
            		...
            		return "/yxxs-wx-portlet/maxinlan/action/actionInfo?actionId=";
            	}
            } },
            ...
            { name: ['createDate', '创建时间'],desc:''
            	,format:function(v,d){
	            	if(d.createDate){
	            		return new Date(v).Format("yyyy-MM-dd hh:mm");//d.createDate;
	            	}else{
	            		return cData;
	            	}
            	} 
            },
            { name: ['startDate', '起始时间'],desc:'',data: '',format:function(v,d){
            	if(d.startDate){
            		return new Date(v).Format("yyyy-MM-dd");//d.startDate;
            	}
            }},
            { name: ['endDate', '结束时间'],desc:'',data: '',format:function(v,d){
            	if(d.endDate){
