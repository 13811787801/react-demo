...
<body>
        ...
        $.ajax({
            ...
            success: function (data) {
			    ...
			    var sUpload = new SingleUpload({
			        upDiv:"#upDiv",
			        userId:${admin['userId']},
			        addFileUrl:'/yxxs-main-portlet/api/addFileSliderShow',
			        uploadStr:"上传（<5M）",
			        coverConfirm:false,
			        sizeLimit:5,
			        ...
			        onComplete:function(){
		                ...
		                $('#imgUrl').val('/yxxs-file-serv-portlet/api/previewFile_'+div.find('.fileId').val());
			        },
			        onDelete:function(){
			        }
			    });
    ...
    <script type="text/javascript">
        var fieldInfoList = [
            { name: ['imgName', '轮播图描述'] },
            { name: ['sliderLink', '连接地址'] },
            { name: ['type', '轮播类型'], type: 'dropdown', data: [{ k: 1, v: '登陆页面轮播图' }] },
        ...
        function initList() {
            ...
            $.ajax({
                url: "/yxxs-main-serv-portlet/api/model/get_SliderShow_<%=id %>",
                type: "POST",
                dataType: "json",
                data: { },
                success: function (data) {
    ...
    function saveData(){
        ...
        hash.id = $('#id').val();
        hash.imgUrl = $('#imgUrl').val();

        $.ajax({
            url: "/yxxs-main-serv-portlet/api/saveSlider",
            type: "POST",
            dataType: "json",
            data: hash,
            success: function (data) {
                alert("保存成功");
                window.location.href='/yxxs-admin-portlet/admin/slider/sliderList';
            }
        });
    }
    </script>
